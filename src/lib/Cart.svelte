<script lang="ts">
	...
	$: {
		...
		for (const item of $cartItemsStore) {
...
>
	<div class="flex items-center justify-between mx-5 py-4">
		<h1 class="uppercase font-bold text-xl">
			...
			{$cartItemsStore.length === 1 ? 'item' : 'items'}
		</h1>
			...
			{#each $cartItemsStore as cartItem}
				<div class="flex mx-5 mb-5 gap-5">
					<img
						...
						width="92"
						height="92"
						alt="{cartItem.title} thumbnail"
					/>

					<div>
						<a href="/products/{cartItem.id}" target="_self">{cartItem.title}</a>
						<div class="flex gap-3 my-2">
							<ProductQuantity bind:count={cartItem.quantity} mini={true} />
							<button
								on:click={() => removeFromCart(cartItem.title)}
